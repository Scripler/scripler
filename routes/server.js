...
exports.status = function (req, res, next) {
	...
	Styleset.findOne({"name": "light-color", "isSystem": true}).exec(function (err, styleset) {
		...
		if (!styleset) {
			return next("Styleset not found!");
		}
		...
		fs.readFile(epubjsReadme, function (err, data) {
			...
			if (!data) {
				return next("EpubJS README.md could not be found. Has submodules been loaded?");
			}
			if (data.length <= 0) {
				return next("EpubJS README.md looks empty. Has submodules been loaded?");
			}
