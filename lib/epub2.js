...
function createMimeType() {
	...
	//console.log('mimetypeTemplateFilename: ' + mimetypeTemplateFilename + "\n");
    var mimeTypeReadStream = fs.createReadStream(mimetypeTemplateFilename);
	archive.append(mimeTypeReadStream, { name: mimetypeFilename, store: false });
}
...
function createMetaInf() {
	...
	//console.log('containerTemplateFilename: ' + containerTemplateFilename + "\n");
	var containerFileReadStream = fs.createReadStream(containerTemplateFilename);
	archive.append(containerFileReadStream, { name: path.join(dirName, containerFilename), store: true });
	...
	//console.log('iBooksTemplateFilename: ' + iBooksTemplateFilename + "\n");
	var iBooksFileReadStream = fs.createReadStream(iBooksTemplateFilename);
	archive.append(iBooksFileReadStream, { name: path.join(dirName, iBooksFilename), store: true });
}
...
function createStylesheet(oepbsTemplateDirName) {
	...
	//console.log("stylesheetTemplateFilename: " + stylesheetTemplateFilename + "\n");
	var stylesheetFileFs = fs.createReadStream(stylesheetTemplateFilename);
	archive.append(stylesheetFileFs, { name: path.join(oepbsTemplateDirName, stylesheetFilename), store: true });
}
...
function createContentOpf(oepbsDirName, documents, title, authors, language, coverImagePath, isbn, uuid, keywords, description) {
	...
	//console.log("contentOpfTemplateFilename: " + contentOpfTemplateFilename + "\n");
	...
	contentOpfFileReadStream.on('data', function (contentOpfData) {
		//console.log('got %d bytes of data', contentOpfData.length);
		//console.log('contentOpfData: ' + contentOpfData + '\n');
		//console.log(typeof contentOpfData, 'string');
		...
		archive.append(contentOpfData, { name: path.join(oepbsDirName, contentOpfFilename), store: true });
	});

	contentOpfFileReadStream.on('error', function (err) {
		//console.log("contentOpfFileReadStream err: " + err);
		throw err;
	});
}
...
function createToC(oepbsDirName, toc, uuid, title, authors) {
	...
	//console.log('tocTemplateFilename' + tocTemplateFilename);
	...
	tocFileReadStream.on('data', function (tocData) {
		...
		//console.log('tocData:\n ' + tocData + '\n');
		archive.append(tocData, { name: path.join(oepbsDirName, tocFilename), store: true });
	});

	tocFileReadStream.on('error', function (err) {
		//console.log("tocFileReadStream err: " + err);
		throw err;
	});
}
...
function createOebps(title, authors, documents, toc, language, coverImagePath, isbn, uuid, keywords, description) {
	...
	filewalker(fontsDirName)
		.on('file', function(p) {
			console.log('dir:  %s', p);
			archive.append(fs.createReadStream(oepbsDirName + '/' + fontsDirName + '/' + p), { name: p, store: true });
		})
	...
	*/
	...
	filewalker(imageDirName)
		.on('file', function(p) {
			console.log('dir:  %s', p);
			archive.append(fs.createReadStream(oepbsDirName + '/' + imageDirName + '/' + p), { name: p, store: true });
		})
		.on('error', function(err) {
			console.error(err);
		})
...
function createArchive (title) {
	var saneTitle = sanitize(title);
	var epubFilename = path.join(resDirName, saneTitle + '.epub');
	//console.log('epubFilename: ' + epubFilename);
	var epubWriteStream = fs.createWriteStream(epubFilename);
...
exports.create = function create (project) {
	...
	//console.log("resDirName: " + resDirName + "\n");
