...
function createContentOpf(oepbsDirName, docPrefix, documents, images, fonts, title, authors, language, coverImagePath, isbn, uuid, keywords, description) {
    ...
    var manifestFontFilesString = getManifestFilesString('font_', 'Fonts', fonts, 'tff', 'application/x-font-ttf'); // TODO: get font files + support other font formats?

	var manifestFontLicenseName = 'Scripler-license.txt';
	var manifestFontLicenseFilename = path.join(resDirName, manifestFontLicenseName);
	var manifestFontLicenseReadStream = fs.createReadStream(manifestFontLicenseFilename);
	archive.append(manifestFontLicenseReadStream, { name: path.join(oepbsDirName, manifestFontLicenseName), store: false });

    var spineDocumentsString = getSpineDocumentsString('doc_', documents, 'html');
    var guideCoverString = '<reference href="HTML/Cover.html" title="Cover" type="cover" />';
    var guideTitlePageString = '<reference href="HTML/TitlePage.html" title="Title Page" type="title-page" />';
    var guideColophonString = '<reference href="HTML/Colophon.html" title="Colophon" type="colophon" />';
    var guideToCString = '<reference href="HTML/ToC.html" title="Table of Contents" type="toc" />';
...
function createOebps(title, authors, documents, toc, language, coverImagePath, isbn, uuid, keywords, description) {
	...
	*/
    ...
    for (var i=0; i<documents.length; i++) {
		...
		var documentFilename = path.join(oepbsDirName, textDirName, documentName);
        archive.append(document.text, { name: documentFilename, store: false });
	}

    // content.opf
    createContentOpf(oepbsDirName, docPrefix, documents, images, fonts, title, authors, language, coverImagePath, isbn, uuid, keywords, description);
