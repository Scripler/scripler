		...
		function(callback) {
			...
			filewalker(fontsSourceDir)
				.on('file', function(fontFile) {
					...
					var fontId = fontPrefix + fontFile;
					fonts.push(fontId);
					archive.append(fs.createReadStream(path.join(fontsSourceDir, fontFile)), { name: path.join(oepbsDir, conf.epub.fontsDir, fontId), store: false });
				})
		...
		function(callback) {
			...
			filewalker(imagesSourceDir)
				.on('file', function(imageFile) {
					...
					var imageId = imgPrefix + imageFile;
					images.push(imageId);
					archive.append(fs.createReadStream(path.join(imagesSourceDir, imageFile)), { name: path.join(oepbsDir, conf.epub.imagesDir, imageId), store: false });
				})
				.on('error', function(err) {
					console.error(err);
					callback(err);
