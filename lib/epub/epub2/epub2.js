...
function createStylesheet(oepbsTemplateDir) {
	...
	var stylesheetTemplateFilename = path.join(templateDir, oepbsTemplateDir, stylesheetFilename);
...
function createContentOpf(oepbsDir, docPrefix, documents, images, fonts, title, authors, language, coverImagePath, isbn, uuid, keywords, description) {
...
function createOebps(userId, projectId, title, authors, documents, toc, language, coverImagePath, isbn, uuid, keywords, description) {
    ...
    for (var i=0; i<documents.length; i++) {
		...
		var documentFilename = path.join(oepbsDir, textDir, documentName);
        archive.append(document.text, { name: documentFilename, store: false });
	}

    // content.opf
    createContentOpf(oepbsDir, docPrefix, documents, images, fonts, title, authors, language, coverImagePath, isbn, uuid, keywords, description);
