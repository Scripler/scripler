...
function createHTML(docPrefix, documents, callback) {
	for (var i = 0; i < documents.length; i++) {
		...
		if (isSystemType(documentType)) {
			var documentTypeName = getDocumentTypeName(documentType);
...
function createContentOpf(docPrefix, imgPrefix, fontPrefix, stylePrefix, fonts, documents, images, styles, title, authors, language, cover, isbn, uuid, keywords, description, rights, publicationDate, type, contributors, publisher, coverage, relation, source, callback) {
	...
	var manifestStyleFilesString = getManifestFilesString(stylePrefix, conf.epub.stylesDir, styles);
