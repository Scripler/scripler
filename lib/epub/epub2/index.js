...
function createHTML(docPrefix, documents, callback) {
	for (var i = 0; i < documents.length; i++) {
		...
		var documentFilename = path.join(conf.epub2.oepbsDir, conf.epub.htmlDir, documentName);
		archive.append(document.text, { name: documentFilename, store: false });
	}
	callback(null);
}

function createContentOpf(docPrefix, imgPrefix, fontPrefix, fonts, documents, images, styles, title, authors, language, cover, isbn, uuid, keywords, description, rights, publicationDate, type, contributors, publisher, coverage, relation, source, callback) {
	var contentOpfFilename = 'content.opf';
	var contentOpfTemplateFilename = path.join(templateDir, conf.epub2.oepbsDir, contentOpfFilename);
	...
	var manifestStyleFilesString = getManifestFilesString(imgPrefix, conf.epub.stylesDir, styles);
	...
	archive.append(manifestFontLicenseReadStream, { name: path.join(conf.epub2.oepbsDir, manifestFontLicenseName), store: false });

	var spineDocumentsString = getSpineDocumentsString('doc_', documents, 'html');
	var guideCoverString = '<reference href="' + conf.epub.htmlDir + '/Cover.html" title="Cover" type="cover" />';
	var guideTitlePageString = '<reference href="' + conf.epub.htmlDir + '/TitlePage.html" title="Title Page" type="title-page" />';
	var guideColophonString = '<reference href="' + conf.epub.htmlDir + '/Colophon.html" title="Colophon" type="colophon" />';
	var guideToCString = '<reference href="' + conf.epub.htmlDir + '/ToC.html" title="Table of Contents" type="toc" />';
	...
	contentOpfFileReadStream.on('data', function (contentOpfData) {
		...
		archive.append(contentOpfData, { name: path.join(conf.epub2.oepbsDir, contentOpfFilename), store: false });
	});
...
function createToC(toc, uuid, title, authors, callback) {
	...
	var tocTemplateFilename = path.join(templateDir, conf.epub2.oepbsDir, tocFilename);
	...
	tocFileReadStream.on('data', function (tocData) {
		...
		archive.append(tocData, { name: path.join(conf.epub2.oepbsDir, tocFilename), store: false });
	});
		...
		function (callback) {
			...
			filewalker(fontsSourceDir)
				.on('file', function (fontFile) {
					...
					fonts.push(font);
					archive.append(fs.createReadStream(path.join(fontsSourceDir, fontFile)), { name: path.join(conf.epub2.oepbsDir, conf.epub.fontsDir, font.id), store: false });
				})
		...
		function (callback) {
			...
			filewalker(imagesSourceDir)
				.on('file', function (imageFile) {
					...
					images.push(image);
					archive.append(fs.createReadStream(path.join(imagesSourceDir, imageFile)), { name: path.join(conf.epub2.oepbsDir, conf.epub.imagesDir, image.id), store: false });
				})
				.on('error', function (err) {
					console.error(err);
					callback(err);
		...
		function (callback) {
			...
			filewalker(stylesSourceDir)
				.on('file', function (styleFile) {
					...
					styles.push(style);
					archive.append(fs.createReadStream(path.join(stylesSourceDir, styleFile)), { name: path.join(conf.epub2.oepbsDir, conf.epub.stylesDir, style.id), store: false });
				})
				.on('error', function (err) {
					console.error(err);
					callback(err);
