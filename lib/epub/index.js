...
var createContentOpf = exports.createContentOpf = function createContentOpf(archive, templateDir, epubDir, epubType, docPrefix, imgPrefix, fontPrefix, stylePrefix, fonts, documents, images, styles, title, authors, language, cover, isbn, uuid, keywords, description, rights, publicationDate, type, contributors, publisher, coverage, relation, source, callback) {
		...
		function (callback) {
			...
			filewalker(imagesSourceDir)
				.on('file', function (imageFile) {
					...
					images.push(image);
					archive.append(fs.createReadStream(path.join(imagesSourceDir, imageFile)), { name: path.join(epubDir, conf.epub.imagesDir, image.id), store: false });
				})
				.on('error', function (err) {
					console.error(err);
					callback(err);
		...
		function (callback) {
			...
			filewalker(stylesSourceDir)
				.on('file', function (styleFile) {
					...
					styles.push(style);
					archive.append(fs.createReadStream(path.join(stylesSourceDir, styleFile)), { name: path.join(epubDir, conf.epub.stylesDir, style.id), store: false });
				})
				.on('error', function (err) {
					console.error(err);
					callback(err);
