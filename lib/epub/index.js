...
var getStylesetLinks = exports.getStylesetLinks = function getStylesetLinks(document) {
	...
	if (document.stylesets) {
		for (var i=0; i<document.stylesets.length; i++) {
			...
			stylesetLinks = stylesetLinks + '<link href="../Styles/' + conf.epub.stylePrefix + stylesetId + '.css" rel="stylesheet" type="text/css"/>';
		}
	}
...
var createHTML = exports.createHTML = function createHTML(docPrefix, documents, htmlDir, epubDir, archive, title, callback) {
	for (var i = 0; i < documents.length; i++) {
		...
		if (project_utils.includeInEbook(document)) {
			...
			var htmlNamespace = '<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops">';
			var headStart = '<head>';
			var headEnd = '</head>';
			var titleTag = '<title>' + title + '</title>';
			var stylesets = getStylesetLinks(document);
			var bodyStart = '<body>';
			var bodyAndHtmlEnd = '</body></html>';
			var html = xmlHeader + docType + htmlNamespace + headStart + titleTag + stylesets + headEnd + bodyStart + document.text + bodyAndHtmlEnd;
