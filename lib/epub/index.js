...
var create = exports.create = function create(epubType, userId, project, archive, templateDir, epubDir, createNav, next) {
	...
	async.series([
		...
		function (callback) {
			...
			//archive.append("sdfsdfsdfsdfs", { name: path.join(epubDir, "jytte.hest"), store: false });
			//callback(null);
		},
		...
		// User fonts - are per user. Exists only in the physical file system and not on the Mongoose model (yet).
		function (callback) {
			...
			filewalker(fontsSourceDir)
				.on('file', function (fontFile) {
					...
					var font = {
						...
						"src": fontPrefix + fontFile.toString(),
						"fileExtension": fileExtension,
						"mediaType": utils.getMediaType(fileExtension)
					};
					fonts.push(font);
