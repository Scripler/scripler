...
var create = exports.create = function create(epubType, userId, project, archive, templateDir, epubDir, createNav) {
	...
	async.series([
		...
		function (callback) {
			...
			var fileExtension = 'css'; // TODO: is this always true?
			var populateStyleset = function (styleset, callback) {
				//Styleset.populate(styleset, {path: 'styles'}, function (err, styleset) {
				//Styleset.findOne({"_id": styleset._id}).populate({path: 'styles'}).exec(function (err, styleset) {
			...
			async.each(project.stylesets, populateStyleset, function(err) {
				if (err) callback(err);
				callback(null);
			});
		},
