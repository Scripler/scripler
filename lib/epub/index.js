...
var create = exports.create = function create(epubType, userId, project, archive, templateDir, epubDir, createNav) {
	...
	async.series([
		...
		// Create Styles dir
		function (callback) {
			// Stylesets - are per project. Exists only in the database (on the Mongoose model).
			var fileExtension = 'css';
			var populateStyleset = function (stylesetId, callback) {
				Styleset.findOne({"_id": stylesetId}).populate({path: 'styles', match: {archived: false}}).exec(function (err, styleset) {
					if (err) callback(err);

					var stylesetContents = styleset_utils.getStylesetContents(styleset);
					if (stylesetContents) {
			...
			if (allStylesets && allStylesets.length > 0) {
				async.each(allStylesets, populateStyleset, function(err) {
					if (err) callback(err);
					callback(null);
				});
			} else {
