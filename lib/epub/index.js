...
var getAuthorString = exports.getAuthorString = function getAuthorString(authors, epubType, next) {
	var authorString = '';

	if (!isValidEPUBType(epubType)) {
		var errorMessage = 'getAuthorString: invalid EPUB type!';
...
var getContributorString = exports.getContributorString = function getContributorString(contributors, epubType, next) {
	var contributorString = '';

	if (!isValidEPUBType(epubType)) {
		var errorMessage = 'getContributorString: invalid EPUB type';
		logger.error(errorMessage);
		return next(errorMessage);
	}
	...
	return next(null, contributorString);
}
...
var createContentOpf = exports.createContentOpf = function createContentOpf(archive, templateDir, epubDir, epubType, docPrefix, imgPrefix, fontPrefix, stylePrefix, fonts, documents, images, stylesets, title, authors, language, cover, isbn, uuid, keywords, description, rights, publicationDate, type, contributors, publisher, coverage, relation, source, callback) {
	...
	getAuthorString(authors, epubType, function (err, authorString) {
		if (err) callback(err);

		var keywordString = getKeywordString(keywords);

		getContributorString(contributors, epubType, function (err, contributorString) {
			if (err) callback(err);
