...
exports.updateOriginalStyleset = function (originalStyleset, newStyleset, next) {
	if (isStylesetPopulated(originalStyleset) && isStylesetPopulated(newStyleset)) {
		console.log("updateOriginalStyleset - stylesets are populated!");
		if (!originalStyleset.isSystem) {
			...
			if (newStyleset.original == originalStyleset.id) {
				...
				// TODO: also update other styleset attributes, e.g. order

				//var originalStyles = originalStyleset.styles;
				console.log('updateOriginalStyleset - originalStyles: ' + JSON.stringify(originalStyles));
				var newStyles = newStyleset.styles;
				//console.log('updateOriginalStyleset - newStyles: ' + JSON.stringify(newStyles));
				...
				if (originalStyleset.styles != null && originalStyleset.styles.length > 0) {
					console.log("ORIGINAL STYLESET PROCESSING - length: " + originalStyleset.styles.length);
					...
					console.log("ORIGINAL STYLESET PROCESSING - length: " + originalStyleset.styles.length + " END!");
				} else {
...
exports.systemStylesetOrder = function (s1, s2) {
	// TODO: implement via s1.order - s2.order?
...
exports.getStylesetOrStyleType = function (stylesetOrStyle, next) {
	...
	if (stylesetOrStyle.isSystem) {
		console.log("getStylesetOrStyleType: STYLESET/STYLE ISSYSTEM = TRUE");
		if (!stylesetOrStyle.original) {
			console.log("getStylesetOrStyleType: STYLESET/STYLE ORIGINAL = FALSE");
			return next(null, "system");
		} else {
			console.log("ERROR: styleset/style has isSystem = true but ALSO an 'original' - this is not allowed! (how did this happen?)");
			return next("ERROR: styleset/style has isSystem = true but ALSO an 'original' - this is not allowed! (how did this happen?)");
		}
	} else {
		console.log("getStylesetOrStyleType: STYLESET/STYLE ISSYSTEM = FALSE");
		if (stylesetOrStyle.original) {
			console.log("getStylesetOrStyleType: STYLESET/STYLE ORIGINAL = TRUE");
