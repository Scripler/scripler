	...
	Styleset.findOne({"original": stylesetToApply._id, "_id": {$in: document.stylesets}}).exec(function (err, styleset) {
		...
		if (shouldCopyStyleset) {
			...
			if (!setAsDefault || utils_shared.mongooseEquals(document.defaultStyleset, stylesetToApply)) {
				...
				document.defaultStyleset = stylesetToApply;
				document.save(function (err) {
					if (err) {
						return next(err);
					}
...
exports.canDeleteDocumentOfType = function (userLevel, documentType) {
	return userLevel == 'premium' || userLevel == 'professional' || (userLevel == 'free' && documentType != 'madewithscripler');
}