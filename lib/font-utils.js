...
/**
 *
 * NB! Not possible to implement via String.replace() (replacing @font-face lines with empty string) because multi-line match ('m') does not work in V8, c.f.
 *
...
function removeFontFromCss(fontSrc, css) {
	if (fontSrc && css) {
		...
		*/
		...
		// TODO: this leaves trailing }s => fix way of calculating when the font-face ends
		var fontFaceEnd = css.lastIndexOf(fontSrc) + fontSrc.length + 1;
		//var fontFaceEnd = postfix.indexOf('}');
		//console.log('fontFaceEnd: ' + fontFaceEnd);

		css = css.slice(0, fontFaceStart) + css.slice(fontFaceEnd);
		return css;

		//css = css.slice(fontFaceStart, fontFaceEnd);   //css.replace(/fontFaceCss/, "");
	}
}
