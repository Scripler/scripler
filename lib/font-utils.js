...
function contains(fonts, fontToCompare) {
	...
	if (fonts && fonts.length > 0) {
		for (var i=0; i<fonts.length; i++) {
			var font = fonts[i];
			//debug('font: ' + JSON.stringify(font));
			//debug('fontToCompare: ' + JSON.stringify(fontToCompare));
...
exports.removeUnusedFonts = function (css, fontsInUse, ebookGeneration, next) {
	...
	//debug('jsonStyles: ' + JSON.stringify(jsonStyles));
	//debug('jsonStyles.length: ' + jsonStyles.length);

	async.each(jsonStyles, function (jsonStyle, callback) {
		//debug('jsonStyle: ' + JSON.stringify(jsonStyle));
		var type = jsonStyle['type'];
		if (type) {
			var declarations = jsonStyle['declarations'];
			//debug('declarations: ' + JSON.stringify(declarations));

			if (type == 'fontface') {
				var font = getFont(declarations);
				var fontInUse = contains(fontsInUse, font);
				if (!fontInUse) {
					//debug('DEBUG: font ' + JSON.stringify(font) + ' not in use, removing it...');
					var fontSrc = declarations['src'];
					css = removeFontFromCss(fontSrc, css);
				} else {
					//debug('DEBUG: font ' + JSON.stringify(font) + ' IS in use, skipping it...');
				}
			}
		...
		if (ebookGeneration) {
			//debug('DEBUG: running in "ebook mode" - adjusting path to "Fonts" directory');
	...
	async.each(jsonStyles, function (jsonStyle, callback) {
		...
		if (type) {
			if (type == 'fontface') {
				...
				//debug('family before: ' + family);
				family = family.replace(/"/g, "");
				//debug('family after: ' + family);
				var font = new Font({
					family: family,
					style: style,
					weight: weight,
				...
				font.save(function (err, font) {
					...
					fonts.push(font);
					debug('Created font: ' + JSON.stringify(font));
					callback(null);
				});
			} else if (type == 'style') {
				debug('Skipping "normal" style...(use "import_system_stylesets" to import these)');
				callback(null);
			}
		} else {
			callback('"type" empty');
...
exports.import_system_fonts = function (alreadyConnected, exitWhenDone, next) {
	...
	//debug(mongoose.connection.host);
	//debug(mongoose.connection.port);
	...
	createFonts(fontsFilename, json, function (err, fonts) {
		...
		//debug('Created and saved fonts ' + fonts);
