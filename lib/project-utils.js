...
exports.documentUsesSVG = function (document) {
	return document && document.text && /<[^>]+ src="[^\"]+\.svg"[^>]*>/.test(document.text);
}
...
exports.generateToCJSON = function (filename, html) {
	...
	for (var i=0; i<tocEntries.length; i++) {
		...
		if (tocEntry.type == 'a') {
			if (i > 0) {
				if (tocEntries[i-1].type == 'a') {
					tocEntries[i].level = tocEntries[i-1].level;
				} else {
					tocEntries[i].level = tocEntries[i-1].level + 1;
				}
			} else {
				tocEntries[i].level = 0;
			}
		} else {
