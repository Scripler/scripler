...
function addProviderToUser(provider, providerId, profile, currentUser, done) {
	...
	User.findOne({email: email}, function (err, user) {
		if (user) {
			...
			// If the user is currently loggedin, we want the modify this usero bject - otherwise we use the database bject.
			if (!currentUser) {
				currentUser = user;
			}
...
function isLoggedIn() {
	return function (req, res, next) {
		if (!req.isAuthenticated()) {
			return res.send({"errorMessage": "User not authenticated"}, 401);
		} else {
			next();
		}
	}
